            ...
            <div class="card-body">
              <form action="{{ $url }}" class="needs-validation" method="post" enctype="multipart/form-data"
                ...
                <div class="row">
                  ...
                  <div class="form-group mb-3 col-md-2 hide-this">
                    <label>Gender</label>
                  ...
                  <div class="form-group mb-3 col-md-2">
                    ...
                    <select name="interested_destination" id="interested_destination" class="form-control select2">
                      <option value="">Select</option>
                      @foreach ($destinations as $row)
                        <option value="{{ $row->destination_name }}"
                          {{ $ft == 'edit' && $sd->interested_destination == $row->destination_name ? 'Selected' : '' }}>
                          {{ $row->destination_name }}</option>
                      @endforeach
                    </select>
                    <span class="text-danger">
                      @error('interested_destination')
                        {{ $message }}
                      @enderror
                    </span>
                  </div>
                  ...
                  <div class="form-group mb-3 col-md-2">
                    <label>D.O.B</label>
                    <input name="date_of_birth" type="date" class="form-control" placeholder="Enter Mobile"
                      value="{{ $ft == 'edit' ? $sd->date_of_birth : old('date_of_birth') }}">
                    <span class="text-danger">
                      @error('date_of_birth')
                        {{ $message }}
                      @enderror
