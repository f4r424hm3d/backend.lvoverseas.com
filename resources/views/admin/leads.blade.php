...
@section('main-section')
                ...
                <div class="row">
                  <div class="col-md-6 col-sm-12 mb-3">
                    <div class="form-group">
                      <label>Search User by Name, Mobile and Email</label>
                      <input name="search" id="search" type="text" class="form-control"
                        placeholder="Search User by Name, Mobile and Email" value="{{ $_GET['search'] ?? '' }}" required>
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-3 col-sm-12 mb-3">
                      <div class="form-group">
                        ...
                        <select name="interested_destination" id="interested_destination"
                          class="form-control js-example-basic-singl">
                          <option value="">Select</option>
                          @foreach ($filterDestinations as $row)
                            <option value="{{ $row->interested_destination }}"
                              {{ isset($_GET['interested_destination']) && $_GET['interested_destination'] == $row->interested_destination ? 'selected' : '' }}>
                              {{ $row->interested_destination }}</option>
                          @endforeach
                        </select>
                      </div>
                    </div>
                    <div class="col-md-3 col-sm-12 mb-3">
                      <div class="form-group">
                        ...
                        <select name="entry_type" id="entry_type" class="form-control js-example-basic-singl">
                          <option value="">Select</option>
                          @foreach ($filterUserTypes as $row)
                            <option value="{{ $row->entry_type }}"
                              {{ isset($_GET['entry_type']) && $_GET['entry_type'] == $row->entry_type ? 'selected' : '' }}>
                              {{ $row->entry_type }}</option>
                          @endforeach
                        </select>
                      </div>
                    </div>
                    ...
                    <tr id="row{{ $row->id }}">
                      ...
                      <td>
                        ...
                        <a href="{{ url('admin/leads/update/' . $row->id) }}"
                          class="waves-effect waves-light btn btn-xs btn-outline btn-info popping-button">
                          <i class="fa fa-edit" aria-hidden="true"></i>
                        </a>

                        <a href="javascript:void()" onclick="DeleteAjax('{{ $row->id }}')"
                          class="waves-effect waves-light btn btn-xs btn-outline btn-danger popping-button">
                      ...
                      <td id="contactTd{{ $row->id }}">
                        ...
                        <i class="fas fa-inbox text-danger"></i> : {{ $row->email }}
                      </td>
                      <td>
  ...
  {{-- @include('admin.update-user-info') --}}
  <script>
    $.ajaxSetup({
      headers: {
